...
//     if (!conductedBy || !ctc || !role || !doj) {
//       toast.error("Please fill all selection details", { position: 'top-right' });
//       return;
//     }

//     // First API call - Update interview status
//     await axios.post("https://hrms-software.onrender.com/UpdateInterview", {
//       ...selectedUser,
//       STATUS: "Selected",
//       CONDUCTED_BY: conductedBy,
//       CTC: ctc,
//       ROLE: role,
//       DOJ: doj  // Add DOJ to the first call if backend expects it
//     });

//     // Second API call - Add to employees/interview list
//     await axios.post("https://hrms-software.onrender.com/addinter", {
//       NAME: selectedUser.NAME,
//       GENDER: selectedUser.GENDER,
//       DATE_OF_BIRTH: selectedUser.DOB?.split("T")[0],
//       MAIL_ID: selectedUser.EMAIL,  // Note: Backend might expect EMAIL instead of MAIL_ID
//       PHONE_NUMBER: selectedUser.PHONE_NUMBER,
//       ADDRESS: selectedUser.ADDRESS,
//       DOI: selectedUser.DOI?.split("T")[0],
//       CTC: ctc,
//       DESIGNATION: role,  // or selectedUser.DESIGNATION?
//       DOJ: doj,
//       CONDUCTED_BY: conductedBy,
//       STATUS: "Selected"
//     });
...
//     await getDetials();
//     toast.success("Candidate Selected Successfully", { position: 'top-right' });
//     setShowSelectModal(false);
...
const Interview = () => {
  ...
  const removeDuplicates = (data) => {
    ...
    data.forEach(item => {
      // Create a unique key using email, phone number, and interview date
      const key = `${item.EMAIL}-${item.PHONE_NUMBER}-${item.DOI}`;
      
      if (!uniqueMap.has(key)) {
        uniqueMap.set(key, item);
      } else {
        // If duplicate found, keep the one with latest timestamp or first occurrence
        console.log('Duplicate found:', item.NAME, item.EMAIL);
      }
    });
  ...
  const getDetials = async () => {
    try {
      ...
      const uniqueData = removeDuplicates(res.data);
      setuser(uniqueData);
      
      // Show message if duplicates were removed
      if (uniqueData.length < res.data.length) {
        console.log(`Removed ${res.data.length - uniqueData.length} duplicate records`);
      }
    } catch (error) {
  ...
  // Additional useEffect to detect duplicates (for debugging)
  useEffect(() => {
    if (user.length > 0) {
      const duplicates = user.filter((item, index, self) =>
        self.findIndex(t => 
          t.EMAIL === item.EMAIL && 
      ...
      if (duplicates.length > 0) {
        console.log('Duplicates detected after filtering:', duplicates);
      }
    }
  }, [user]);
